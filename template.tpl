___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Insider Product Information",
  "categories": ["CONVERSIONS","ATTRIBUTION","ANALYTICS"],
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
    "thumbnail": "data:image/png;base64,\u003d\u003d"
  },
  "description": "This template collects product information on any product page.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "productId",
    "displayName": "Product Id Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product id. It interpret dots as nested values. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "productDetail.id"
  },
  {
    "type": "TEXT",
    "name": "productName",
    "displayName": "Product Name Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product name. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "productDetail.name"
  },
  {
    "type": "TEXT",
    "name": "productPrice",
    "displayName": "Product Price Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product price.For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "productDetail.price"
  },
  {
    "type": "TEXT",
    "name": "productOriginalPrice",
    "displayName": "Product Original Price Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product original price. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "productDetail.originalPrice"
  },
  {
    "type": "TEXT",
    "name": "productUrl",
    "displayName": "Product Url Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product url. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "productDetail.url"
  },
  {
    "type": "TEXT",
    "name": "productImage",
    "displayName": "Product Image Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product image. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "productDetail.image"
  },
  {
    "type": "TEXT",
    "name": "productQuantity",
    "displayName": "Product Quantity Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product quantity. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required."
      }
    ],
    "defaultValue": "productDetail.quantity"
  },
  {
    "type": "TEXT",
    "name": "productCategories",
    "displayName": "Product Categories Variable Name",
    "simpleValueType": true,
    "help": "You should write the variable name which keeps product categories. For example, interpret \"a.b.c\" as three nested levels: {a: {b: {c: \"value\"}}}.",
    "valueValidators": [
      {
        "type": "NON_EMPTY",
        "errorMessage": "This input required.",
        "enablingConditions": []
      }
    ],
    "defaultValue": "productDetail.categories"
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const copyFromWindow = require('copyFromWindow');
const setInWindow = require('setInWindow');

const insiderObject = copyFromWindow('insider_object') || {};
const dataLayer = copyFromWindow('dataLayer');

var getData = function (data) {
    var dataFieldPath = data.split('.');
    var parentDataName = dataFieldPath[0];
    var parentData = {};

    dataFieldPath.shift();

    dataLayer.some(function (windowData) {
        if (typeof windowData[parentDataName] !== 'undefined') {
            parentData = windowData[parentDataName];

            return true;
        }
    });

    if (typeof parentData !== 'undefined') {
        dataFieldPath.forEach(function (field) {
            parentData = parentData[field];
        });
    }

    return parentData;
};

insiderObject.product = insiderObject.product || {};

insiderObject.product.id = getData(data.productId) || '';
insiderObject.product.name = getData(data.productName) || '';
insiderObject.product.taxonomy = getData(data.productCategories) || [];
insiderObject.product.unit_sale_price = getData(data.productPrice) || '0';
insiderObject.product.unit_price = getData(data.productOriginalPrice) || '0';
insiderObject.product.url = getData(data.productUrl) || '';
insiderObject.product.stock = getData(data.productQuantity) || '0';
insiderObject.product.product_image_url = getData(data.productImage) || '';

setInWindow('insider_object', insiderObject, true);

data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "dataLayer"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "insider_object"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 11/20/2020, 3:03:23 PM


